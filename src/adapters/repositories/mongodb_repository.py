...
class PyMongoRepository:
    ...
    def search_by_id(self, model_id: str) -> Dict[str, Any] | None:
        return self._collection.find_one({'_id': ObjectId(model_id)}, session=self._session)

    def save(self, data: Dict[str, Any]) -> ObjectId:
        return self._collection.insert_one(data, session=self._session).inserted_id
    
    def update(self, model_id, data: Dict[str, Any]) -> Dict[str, Any]:
        return self._collection.update_one({'_id': ObjectId(model_id)}, data, upsert=False, session=self._session)
